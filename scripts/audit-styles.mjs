...
const allowedVueStyle = "src/.vitepress/theme/views/ArticlePage.vue";
...
const builtinPalette = "slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose";
const colorUtility = "text|bg|border|ring|outline|fill|stroke|decoration|divide";
const builtinColorPattern = new RegExp(`\\b(?:${colorUtility})-(?:(?:${builtinPalette})-(?:50|100|200|300|400|500|600|700|800|900|950)|black|white)(?:/\\d+)?\\b`, "g");
const builtinProsePattern = new RegExp(`\\b(?:prose-(?:${builtinPalette})|prose-invert)\\b`, "g");
const arbitraryColorPattern = new RegExp(`\\b(?:${colorUtility})-\\[(?:#|(?:rgb|rgba|hsl|hsla|hwb|lab|lch|oklab|oklch|color)\\()`, "gi");
const legacyColorName = "canvas|surface(?:-subtle)?|ink(?:-muted|-subtle)?|line(?:-strong)?|accent-(?:strong|soft)|notice-[\\w-]+|draft-[\\w-]+|scrim(?:-soft)?|preview-[\\w-]+|photo-[\\w-]+|live-(?:surface|ink)|code-(?:surface(?:-raised)?|line|muted|subtle|ink|control-(?:surface|line|ink)|accent-focus|add(?:-ink)?|remove(?:-ink)?)";
const legacyTokenPattern = new RegExp(`--color-(?:${legacyColorName})(?![\\w-])|\\b(?:bg|text|border|ring|outline|fill|stroke|decoration|divide)-(?:${legacyColorName})(?![\\w-])`, "g");
const replaceableArbitraryPattern = /\b(?:text|tracking|backdrop-blur|aspect)-\[[^\]\r\n]+\]|\banimate-\[spin[^\]\r\n]*\]/g;
...
const pairedColors = [
  ...
  ...["card", "popover", "primary", "secondary", "muted", "accent", "info", "success", "important", "warning", "destructive", "caution"]
    .map(name => [`--color-${name}`, `--color-${name}-foreground`]),
  ...["background", "card", "inserted", "deleted"]
    .map(name => [`--color-code-${name}`, `--color-code-${name === "background" ? "foreground" : `${name}-foreground`}`]),
];
...
for (const file of await walk(sourceRoot)) {
  ...
  if (authoredSourceExtensions.has(extension)) {
    ...
    if (builtinColor)
      failures.push(`${name}: 不允许 Tailwind 内置颜色 ${builtinColor}`);
    const arbitraryColor = content.match(arbitraryColorPattern)?.[0];
    if (arbitraryColor)
      failures.push(`${name}: 不允许任意硬编码颜色 ${arbitraryColor}`);
    const legacyToken = content.match(legacyTokenPattern)?.[0];
    if (legacyToken)
      failures.push(`${name}: 不允许旧设计令牌 ${legacyToken}`);
    const replaceableArbitrary = content.match(replaceableArbitraryPattern)?.[0];
    if (replaceableArbitrary)
      failures.push(`${name}: ${replaceableArbitrary} 必须改用现有命名令牌`);
  }
  ...
  if (extension === ".vue") {
    ...
    if (hasStyleTag && name !== allowedVueStyle) {
      failures.push(`${name}: 仅 ${allowedVueStyle} 允许 Vue <style scoped>`);
    }
    else if (hasStyleTag) {
      if (styleBlocks.length !== 1 || !/\bscoped\b/i.test(styleBlocks[0]?.[1] ?? ""))
        failures.push(`${name}: Markdown 适配层必须且只能包含一个 <style scoped>`);
    ...
    if (/\sstyle\s*=/.test(content))
      failures.push(`${name}: 不允许内联 style 属性`);
    if (/styled\.|css`|useCssModule/.test(content))
      failures.push(`${name}: 不允许 CSS-in-JS 或 CSS Modules`);
...
const fontRules = fontCss
  ...
  .trim();
if (fontRules || !fontCss.includes("@font-face"))
  failures.push(`${fontPackageCss}: 字体包入口只允许 @font-face`);
...
for (const [index, rawLine] of tailwind.split(/\r?\n/).entries()) {
  ...
  if (line.startsWith("--color-") && line.endsWith(";")) {
    ...
    if (property !== "--color-*") {
      if (!/^oklab\([^;]+\)$/.test(value))
        failures.push(`${allowedCss}:${index + 1}: ${property} 必须使用 oklab()`);
  ...
  if (!line || line === "}" || line.startsWith("@") || line.startsWith("--") || inNamedUtility)
    continue;
  failures.push(`${allowedCss}:${index + 1}: Tailwind 入口只允许指令和主题令牌`);
}

if (!tailwind.includes("--color-*: initial;"))
  failures.push(`${allowedCss}: 必须关闭 Tailwind 默认颜色命名空间`);
...
for (const property of darkThemeColors) {
  if (!themeColors.has(property))
    failures.push(`${allowedCss}: theme-dark 只能覆盖已定义的同名语义颜色 ${property}`);
}
for (const property of requiredThemeTokens) {
  if (!themeTokens.has(property))
    failures.push(`${allowedCss}: 缺少设计系统令牌 ${property}`);
}
