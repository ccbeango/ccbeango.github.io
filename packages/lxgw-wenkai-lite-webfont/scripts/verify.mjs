...
if (version !== source.tag)
  failures.push(`VERSION ${version} does not match source tag ${source.tag}`);
...
if (packageJson.types !== "./index.d.ts")
  failures.push("Package types must point to ./index.d.ts");
if (packageJson.exports?.["."]?.types !== "./index.d.ts")
  failures.push("Package root types must export ./index.d.ts");
...
const remainder = css
  ...
  .trim();
if (remainder)
  failures.push("index.css may only contain @font-face rules");
if (rules.length !== source.assets.length * Object.keys(unicodeRanges).length)
  failures.push(`Expected ${source.assets.length * Object.keys(unicodeRanges).length} font-face rules, found ${rules.length}`);
...
if (referencedFiles.length !== rules.length)
  failures.push("Every font-face rule must reference one local WOFF2 file");
