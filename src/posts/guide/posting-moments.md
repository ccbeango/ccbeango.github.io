...
可选字符串不能是空字符串。`date` 和 `updated` 推荐使用 `YYYY-MM-DD HH:mm:ss`，例如 `2021-12-18 13:58:12`；不需要填写时区，构建期会按站点默认的 `Asia/Shanghai` 解析。只填写 `YYYY-MM-DD` 时按当天 `00:00:00` 处理。日期、图片数量、图片路径和替代文本会在构建期校验，错误会指出对应动态和字段。
