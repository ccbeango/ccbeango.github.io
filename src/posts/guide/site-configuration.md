...
配置单元测试只检查非空文本、有效地址、正整数、派生行为和本地资源是否存在，不固定站点名称、个人文案、账号、导航内容、数量取值或 Giscus ID。正常修改这些配置时无需同步改写测试期望；只有新增配置结构或改变配置规则时才需要更新测试。
...
博客使用 [Giscus](https://giscus.app/zh-CN) 将 GitHub Discussions 作为评论存储。站点不需要数据库或评论服务端；读者可以公开阅读评论，使用 GitHub 账号登录后参与讨论。

`siteConfig.giscus` 的类型是 `GiscusConfig | null`。不需要评论时保持 `null`；需要评论时，按照下面的流程准备 GitHub 仓库并获取配置参数。

### 准备 GitHub 仓库

1. 确保承载评论的 GitHub 仓库是公开仓库。评论仓库可以是博客源码仓库，也可以是单独创建的公开仓库。
2. 打开仓库的 `Settings`，在 `General > Features` 中启用 `Discussions`。
3. 打开 [Giscus GitHub App](https://github.com/apps/giscus)，选择 `Install` 或 `Configure`，并授权它访问承载评论的仓库。只授权实际使用的仓库即可。
4. 在仓库的 `Discussions` 页面确认至少有一个可用分类。推荐使用 `Announcements`，这样普通用户不能脱离文章随意创建新的主讨论。

如果 Giscus 配置页无法识别仓库，优先检查仓库是否公开、Discussions 是否启用，以及 Giscus App 是否已经获得该仓库的访问权限。

### 生成 Giscus 参数

打开 [Giscus 配置页](https://giscus.app/zh-CN)，完成以下设置：

1. 在“仓库”中填写 `owner/repository`。页面显示仓库满足条件后再继续。
2. 在“页面 ↔️ Discussions 映射关系”中选择 pathname 对应的方案。本项目推荐 `pathname`，同一路径始终使用同一条 Discussion；文章 slug 改变后会被视为一篇新文章。
3. 在“Discussion 分类”中选择前面准备的分类，通常使用 `Announcements`。
4. 根据需要开启主贴反应，将评论框位置设为底部，并选择简体中文。
5. 在页面底部找到生成的 `<script>`。不需要把这段脚本粘贴进 Vue 组件，只需把其中的 `data-*` 属性转换为 `site.config.ts` 的配置。

| Giscus `<script>` 属性   | `siteConfig.giscus` 字段 | 示例值             |
| ------------------------ | ------------------------ | ------------------ |
| `data-repo`              | `repo`                   | `owner/repository` |
| `data-repo-id`           | `repoId`                 | `R_...`            |
| `data-category`          | `category`               | `Announcements`    |
| `data-category-id`       | `categoryId`             | `DIC_...`          |
| `data-mapping`           | `mapping`                | `pathname`         |
| `data-reactions-enabled` | `reactionsEnabled`       | `1`                |
| `data-input-position`    | `inputPosition`          | `bottom`           |
| `data-lang`              | `lang`                   | `zh-CN`            |

生成脚本中的 `data-theme`、`data-strict`、`data-emit-metadata` 和 `data-loading` 不需要写入站点配置：项目已经统一管理这些行为。

### 写入站点配置

打开 `src/.vitepress/site.config.ts`，将默认的 `giscus: null as GiscusConfig | null` 替换为完整配置：
...
不要根据仓库名称猜测 `repoId` 或 `categoryId`，这两个值必须从 Giscus 为目标仓库生成的脚本中复制。若以后更换评论仓库或 Discussion 分类，应回到配置页重新生成全部参数。

全部字段有效时，评论区会出现在文章正文下方；首页、文章列表、标签、归档和动态页不会加载评论。组件固定使用 `strict="0"`、`emit-metadata="0"` 和 `loading="lazy"`，并在评论区接近视口时才加载 Giscus。评论主题会自动跟随 VitePress 的 `appearance` 状态，不需要在配置中重复声明。

### 验证评论功能

1. 执行 `pnpm dev`，打开任意文章详情页并滚动到正文底部。
2. 确认评论框能够加载，登录 GitHub 后发送一条测试评论。
3. 打开评论仓库的 `Discussions`，确认对应文章已经产生 Discussion。
4. 执行 `pnpm exec vitest run tests/unit/site-config.test.ts`，检查配置字段和 ID 格式。

首次发表评论时，Giscus 才会为当前 pathname 创建 Discussion，这是正常行为。如果评论区没有显示，可依次检查 `giscus` 是否仍为 `null`、字段是否全部复制、Giscus App 授权、Discussions 开关、仓库可见性，以及浏览器网络或内容拦截扩展是否阻止了 `giscus.app`。

需要关闭评论时，将配置恢复为 `giscus: null` 即可，不需要删除组件或卸载依赖。已有 Discussion 会保留在 GitHub 中，但站点不再加载它们。
