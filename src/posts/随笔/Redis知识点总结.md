...
3. Redis持久化的含义，Redis持久化的方式以及优缺点：
   ...
   - RDB：是`Redis Database`的缩写，在指定的时间间隔对数据进行快照存储。
     ...
     ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结01.png)
   ...
   - AOF是`Append-only file`的缩写
     ...
     ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结02.png)
...
8. 缓存雪崩：当缓存服务器重启或者大量缓存集中在某一个时间段失效，发生大量的缓存穿透，会给后端系统带来很大压力。导致系统崩溃。
   ...
   - 缓存永不过期：包含两层含义，(1) 从缓存上看，确实没有设置过期时间，这就保证了，不会出现热点key过期问题，也就是“物理”不过期。 (2) 从功能上看，如果不过期，那不就成静态的了吗？所以我们把过期时间存在key对应的value里，如果发现要过期了，通过一个后台的异步线程进行缓存的构建，也就是“逻辑”过期。
     ...
     ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结03.png)
...
16. Redis架构模式以及特点：
    ...
    ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结04.png)
    ...
    **2.主从复制：**
    ...
    ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结05.png)
    ...
    **3.哨兵：**
    ...
    ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结06.png)
    ...
    **4.集群（proxy 型）：**
    ...
    ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结07.png)
    ...
    **5.集群（直连型）：**
    ...
    ![文章配图](https://cdn.jsdelivr.net/gh/ccbeango/blogImages/Redis/Redis知识点总结08.png)
