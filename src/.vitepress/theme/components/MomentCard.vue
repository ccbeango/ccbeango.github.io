...
import type { MomentData } from "../../data/moment-types";
...
const hasPlayableMedia = computed(() =>
  props.moment.content.some(({ type }) => type === "music" || type === "video" || type === "live-photo"),
);
...
watch(() => props.moment.content, resetAndMeasure);
...
<template>
  ...
  >
    ...
    <div class="min-w-0">
      ...
      >
        <template
          v-for="(block, index) in moment.content"
        ...
        >
          ...
          <MusicCard
            ...
            :artist="block.artist"
            :cover="block.cover"
            :cover-alt="block.coverAlt"
          />
          <LivePhoto
            v-else-if="block.type === 'live-photo'"
            :poster="block.poster"
            :video="block.video"
            :mode="block.mode"
            :android-source="block.androidSource"
            :alt="block.alt"
          />
          <VideoPlayer
            v-else
