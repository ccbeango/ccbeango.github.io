...
export function formatExposureTime(value: number | undefined) {
  ...
  if (value < 1) {
    ...
    const display = Math.abs(denominator - rounded) < 0.05
      ? String(rounded)
...
function joinCamera(make: string | undefined, model: string | undefined) {
  ...
  if (!model)
    return make;
  if (model.toLocaleLowerCase().includes(make.toLocaleLowerCase()))
    return model;
  return `${make} ${model}`;
}

export function formatPhotoMetadata(record: ExifRecord | undefined, input: PhotoMetadataInput): PhotoMetadata {
  const width = numberValue(record?.ExifImageWidth)
    ?? numberValue(record?.ImageWidth)
    ?? input.width;
  const height = numberValue(record?.ExifImageHeight)
  ...
  return {
    ...
    exposureBias: exposureBias !== undefined
      ? `${exposureBias > 0 ? "+" : ""}${formatNumber(exposureBias, 2)} EV`
      : undefined,
    hasShootingData,
  };
}
...
async function readPhotoMetadata(input: PhotoMetadataInput) {
  ...
  if (!response.ok)
    throw new Error(`照片请求失败：${response.status}`);
