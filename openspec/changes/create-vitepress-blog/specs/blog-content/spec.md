...
系统 SHALL 使用 `::: link-card` 与 `:::` 包围文章引用。区块第一段 MUST 且只能包含一条标准 Markdown 链接，其链接文字 SHALL 成为卡片标题；第二段 MAY 包含一段纯文本说明。站内链接 MUST 使用绝对路径并应用 VitePress `base`，完整 HTTP/HTTPS 链接 SHALL 保持不变。起始行 MUST NOT 包含参数。

#### Scenario: 渲染文章引用

- **WHEN** 合法 `link-card` 容器包含一条 Markdown 链接和可选说明
- **THEN** 页面使用现有文章引用卡片展示标题、说明和可访问链接

#### Scenario: 拒绝无效文章引用

- **WHEN** 起始行包含参数、链接缺失或多于一条、使用相对或不安全地址、说明包含富文本、存在额外段落、嵌套或缺少结束 `:::`
...
系统 SHALL 接受 `::: video <source>` 通用视频容器，并 SHALL 使用兼容站点 base path 与完整远程 URL 的原生视频播放器。播放器 MUST 提供浏览器 controls、`playsinline` 与 `preload="metadata"`，MUST NOT 自动播放。容器 MAY 为空，或包含一张普通 Markdown 图片作为 poster 与可访问标题；视频 poster MUST NOT 进入文章照片预览序列。正文、多个 poster、嵌套容器、额外参数或缺少结束 `:::` MUST 使 Markdown 构建失败并报告文件、行号和具体原因。

#### Scenario: 渲染带封面的视频

- **WHEN** 合法 `video` 容器提供视频地址和一张 Markdown 封面图片
- **THEN** 页面展示具有原生播放、进度、音量与全屏能力的响应式视频播放器，并使用图片地址与 alt 作为 poster 和可访问标题

#### Scenario: 渲染无封面的视频

- **WHEN** 合法 `video` 容器只提供视频地址
- **THEN** 页面展示使用默认可访问标题的原生视频播放器

#### Scenario: 拒绝无效视频容器

- **WHEN** `video` 容器缺少地址、包含额外参数、正文、多张图片、嵌套区块或缺少结束 `:::`
- **THEN** Markdown 处理失败并报告源文件、行号和具体原因

### Requirement: 三冒号远程音乐

系统 SHALL 接受 `::: music <remote-source> | <title> | <artist>` 音乐容器，并 SHALL 允许区块内包含一张可选的普通 Markdown 图片作为封面。普通远程音频直链与 Motues `type=url` 模式 MUST 提供非空歌曲名和歌手。Motues `type=details` 模式 SHALL 只接受 URL，并 SHALL 由客户端自动读取歌曲名、歌手、专辑和封面；作者 MAY 提供 Markdown 图片覆盖自动封面。Motues URL MUST 使用 `/music`、非空 `id`、`details` 或 `url` type，以及可选的 `netease`、`tencent`、`kugou`、`baidu` 或 `kuwo` server；server 省略时 SHALL 使用接口默认值。系统 MUST NOT 在构建阶段请求音乐接口，也 MUST NOT 要求仓库保存音乐资源。正文、多张封面、嵌套容器、本地音频路径或缺少结束 `:::` MUST 使 Markdown 构建失败并报告文件、行号和具体原因。

#### Scenario: 使用远程音频直链

- **WHEN** 合法 `music` 容器提供完整远程音频 URL、歌曲名、歌手和可选封面
- **THEN** 页面渲染对应音乐卡片，且初始加载不请求音频

#### Scenario: 使用 Motues details 自动填充

- **WHEN** 合法 `music` 容器只提供 `open.motues.top/music` 的 `type=details` 地址
- **THEN** 页面标记该来源为自动元数据模式，并在客户端使用 details、cover 和 url 接口完成展示与播放

#### Scenario: 使用 Motues url 手工填充

- **WHEN** 合法 `music` 容器提供 `type=url` 地址、歌曲名和歌手
- **THEN** 页面使用 Markdown 展示信息，并在播放时解析该临时音频地址

#### Scenario: 拒绝无效音乐容器

- **WHEN** `music` 容器使用本地路径、不支持的 Motues server 或 type、在 details 模式额外填写标题、在直链或 url 模式缺少歌曲名或歌手、包含正文、多张图片、嵌套区块或缺少结束 `:::`
- **THEN** Markdown 处理失败并报告文件、行号和具体原因
...
项目 SHALL 将快速开始与工程规范、站点配置、文章写作、Markdown 扩展、远程音视频、图片布局、Live Photo 和部署说明作为 `src/posts/guide` 下的正式博客文章，并 SHALL 通过自动多分组 sidebar 形成连续使用手册。工程规范 MUST 说明 ESLint 质量检查、Prettier 格式化、Tailwind class 规范化、编辑器集成和命令行检查。README SHALL 只保留仓库入口、命令摘要和手册索引；项目 MUST NOT 维护内容重复的 `docs` 或无说明价值的测试文章。手册中的真实示例 SHALL 同时用于自动化回归。普通图片、文章封面、图片网格和显式 MP4 海报 SHALL 复用同一张通用演示图片，不得为不同布局复制图片文件；Android Motion Photo MAY 另保留一张真实功能样本；音乐 SHALL 使用远程来源且 MUST NOT 增加本地音频样本。

#### Scenario: 阅读并验证使用手册

- **WHEN** 用户访问 `/blog/guide/*` 或测试验证工程规范、Markdown、系列、音视频、图片与 Live Photo
- **THEN** 同一组手册文章提供当前工具与功能的完整用法，也作为对应功能的测试样例，并只保留一张共享通用演示图片与一张 Android 功能样本
