...
- 将 Tailwind CSS 4 作为主要 authored UI styles 来源，使用基于 `oklab()` 的语义颜色和统一设计令牌；仅在文章页使用一个采用标准 CSS Nesting 的 scoped CSS 适配层整理 VitePress 生成的 Markdown DOM 样式，不增加 nesting 插件或额外 PostCSS 配置；使用 ESLint Flat Config、`@antfu/eslint-config` 与 `eslint-plugin-better-tailwindcss` 同时代码格式化、代码换行、canonical class 和官方 class order，不引入 Prettier 或外部 formatter。
- 支持 VitePress 官方 Markdown 扩展、数学公式、任务列表、三冒号 `image-grid` 和 `live-photo` 容器、Android Motion Photo、统一照片预览、缩放平移和按需 EXIF 信息。
...
- 将站点使用说明作为 `src/posts/guide` 中的正式系列文章维护，并同时作为 Markdown 与浏览器回归样例。
...
- 字体资产由 `packages/lxgw-wenkai-lite-webfont` 管理，代码和样式规范由根级 ESLint 配置及样式审计脚本执行。
