...
test.describe("动态页面", () => {
  ...
  test("按构建期内容块渲染富媒体且不输出内部 marker", async ({ page }) => {
    await page.goto("/moment#moment-2026-shared-reading");

    const card = page.locator("#moment-2026-shared-reading");
    await expect(card).toBeVisible();
    await expect(card.getByRole("link", { name: "使用 Markdown 扩展" })).toBeVisible();
    expect(await page.locator("body").innerHTML()).not.toContain("bean-moment-rich-media");
  });
});
