...
describe("vitePress 官方 Markdown 扩展", () => {
  ...
  it("保留容器、Alert、Code Group 与代码行状态的官方 markup", async () => {
    const article = await readFile(new URL("../../src/posts/guide/markdown-extensions.md", import.meta.url), "utf8");
    const source = matter(article).content;

    const html = await markdown.renderAsync(source, { path: "guide/markdown-extensions.md" });

    for (const type of ["info", "tip", "warning", "danger"]) expect(html).toContain(`class="${type} custom-block"`);
    ...
    for (const title of ["信息", "提示", "警告", "危险", "备注", "重要", "注意"]) expect(html).toContain(`>${title}</`);
  ...
  it("使用官方 image plugin 为普通图片增加懒加载", async () => {
    const html = await markdown.renderAsync("![Markdown 图片演示](/media/live-photo-sample-poster.png)", {
      path: "lazy-image.md",
    });
