...
function entry(slug: string, frontmatter: Record<string, unknown> = {}, html = "<p>一条短动态</p>"): MomentContentData {
  return {
    url: `/moments/${slug}`,
    src: "一条短动态",
    html,
...
describe("动态 frontmatter 与标识", () => {
  it("保留嵌套 slug 并填充默认值", () => {
    expect(toMomentData(entry("life/evening-walk"))).toMatchObject({
      ...
      pinned: false,
      draft: false,
      content: [{ type: "html", html: "<p>一条短动态</p>" }],
    });
  });
...
describe("动态 Markdown", () => {
  ...
  it("渲染受支持富媒体，并将容器封面排除在动态图库之外", async () => {
    ...
    const content = renderMomentContentBlocks(markdown, tokens, env);
    const html = content
      .filter((block) => block.type === "html")
      .map((block) => block.html)
      .join("");

    expect(html).toContain("今天想分享四件事");
    expect(JSON.stringify(content)).not.toContain("bean-moment-rich-media");
    ...
    expect(content).toMatchObject([
      { type: "html", html: expect.stringContaining("今天想分享四件事") },
      { type: "link-card", href: "/blog/guide/markdown-extensions", title: "使用 Markdown 扩展" },
      {
        type: "music",
    ...
    await expect(
      markdown.renderAsync("::: video /media/live-photo-sample.mp4\n:::", {
        relativePath: "posts/article.md",
      }),
    ).resolves.toContain("<VideoPlayer");
