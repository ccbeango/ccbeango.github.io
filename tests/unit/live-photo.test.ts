...
const validBlock = ["::: live-photo /media/lake.mp4", "![湖面的倒影](/images/lake.jpg)", ":::"].join("\n");

const androidBlock = ["::: live-photo android", "![清晨的街道](/live-images/motion-photo.jpg)", ":::"].join("\n");
...
describe("live-photo Markdown plugin", () => {
  ...
  it("uses the Android Motion Photo JPEG as both poster and source", async () => {
    ...
    expect(html).toContain(
      '<LivePhoto mode="android" poster="/live-images/motion-photo.jpg" android-source="/live-images/motion-photo.jpg" alt="清晨的街道" />',
    );
    expect(html).not.toContain("<img ");
  });
  ...
  it("does not enhance the removed HTML comment syntax", async () => {
    const html = await render(
      enhanced,
      ["<!-- live-photo: android -->", "![清晨的街道](/live-images/motion-photo.jpg)", "<!-- /live-photo -->"].join(
        "\n",
      ),
    );
  ...
  it.each([
    ...
    [
      "Android mode 带有额外参数",
      "::: live-photo android /media/a.mp4\n![图片](/images/one.jpg)\n:::",
      "必须提供非空 MP4 地址或 android mode",
    ],
